...
async function testModel(modelKey) {
  const service = AIServiceFactory.createServiceByName("OPENAI", modelKey);
...
async function main() {
  if (!process.env[AI_PROVIDERS.OPENAI.apiKeyEnvVar]) {
    console.error(
      `Missing ${AI_PROVIDERS.OPENAI.apiKeyEnvVar}. Skipping OpenAI integration tests.`
    );
    process.exit(0);
  }

  const results = [];
  for (const modelKey of Object.keys(AI_PROVIDERS.OPENAI.models)) {
    try {
      process.stdout.write(`OpenAI ${modelKey}: `);
  ...
  if (failed.length > 0) {
    console.error(`\n${failed.length} OpenAI model(s) failed.`);
    process.exit(1);
  }

  console.log("\nAll OpenAI models returned OK.");
}
