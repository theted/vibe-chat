...
async function testModel(modelKey) {
  const service = AIServiceFactory.createServiceByName("KIMI", modelKey);
...
async function main() {
  if (!process.env[AI_PROVIDERS.KIMI.apiKeyEnvVar]) {
    console.error(
      `Missing ${AI_PROVIDERS.KIMI.apiKeyEnvVar}. Skipping Kimi integration tests.`
    );
    process.exit(0);
  }

  const results = [];
  for (const modelKey of Object.keys(AI_PROVIDERS.KIMI.models)) {
    try {
      process.stdout.write(`Kimi ${modelKey}: `);
  ...
  if (failed.length > 0) {
    console.error(`\n${failed.length} Kimi model(s) failed.`);
    process.exit(1);
  }

  console.log("\nAll Kimi models returned OK.");
}
