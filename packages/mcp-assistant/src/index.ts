  ...
  async ensureCollection(): Promise<boolean> {
    ...
    try {
      ...
      if (LangChainMcpServer.#isMissingCollection(error)) {
        ...
        console.warn(
          `[MCP] To create and populate the collection, run: bun run build && bun dist/scripts/index-mcp-chat.js`,
        );
        console.warn(
  ...
  async answerQuestion(
    ...
    try {
      ...
      if (LangChainMcpServer.#isMissingCollection(error as ErrorWithCode)) {
        return {
          answer:
            "I could not find an indexed knowledge base. Run `bun run build && bun dist/scripts/index-mcp-chat.js` to build it, then ask again.",
          contexts: [],
        };
      }
...
export { DEFAULT_CHROMA_URL, DEFAULT_COLLECTION_NAME } from "./constants.js";
