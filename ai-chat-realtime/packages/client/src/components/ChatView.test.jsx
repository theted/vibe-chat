...
vi.mock('./MessageInput.jsx', () => ({
  default: ({ onSendMessage, disabled }) => (
    <div data-testid="message-input">
      <button onClick={() => onSendMessage('test')} disabled={disabled}>Send</button>
    </div>
  ),
}));
...
vi.mock('./TypingIndicator.jsx', () => ({
  default: ({ typingUsers, typingAIs }) => (
    <div data-testid="typing-indicator">{typingUsers.length} users, {typingAIs.length} AIs typing</div>
  ),
}));

vi.mock('./Icon.jsx', () => ({
  default: ({ name, className }) => (
    <span data-testid={`icon-${name}`} className={className}>Icon-{name}</span>
  ),
}));
...
describe('ChatView Component', () => {
  const defaultProps = {
    ...
    messages: [{ id: 1, text: 'Hello', username: 'user1' }, { id: 2, text: 'Hi there', username: 'user2' }],
    typingUsers: [],
    typingAIs: [],
  ...
  describe('scroll button', () => {
    it('should show scroll button when showScrollButton is true', () => {
      ...
      renderWithRouter(<ChatView {...props} />);
      const scrollButton = screen.getByTestId('icon-arrow-down').closest('button');
  ...
  describe('error handling', () => {
    it('should display error message when error exists', () => {
      const props = { ...defaultProps, error: 'Connection failed. Please try again.' };
      renderWithRouter(<ChatView {...props} />);
